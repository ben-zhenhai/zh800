        ...
        if(WriteFileCount == 299 || PrintLeftLog == 1)
        {
            ...
            for(ForCount = 0; ForCount <= WriteFileCount; ForCount++)
            {
                ...
                if(LogString[ForCount] == '\n')
                {
                    ...
                    ts = *localtime(&now.tv_sec);
                    strftime(buff, sizeof(buff), "%Y/%m/%d_%H:%M:%S", &ts);
                    fprintf(pfile,"tsw100\t%s\t%ld\t",buff,PINCount[0][6]);
                }
            }      
...
//            printf("%d \n",WatchDogCoolDown);
        ...
        for(ForCount = 0; ForCount < 5; ++ForCount)
        {
            ...
            {
                if(PINEXCount[ForCount][ForCount2] != PINCount[ForCount][ForCount2] && ForCount2 == 6 && ForCount == 0)
                {
                    fprintf(pfile, "%s\t%s\t%s\t%ld\t%ld\t0\t%s\t%d\t%s\t%s\t0\t0\t0\t%02d\n", ISNo, ManagerCard, CountNo, PINCount[0][6], (long)now.tv_sec,
...
void * zhINTERRUPT1(void * argument)
{
    while(zhInterruptEnable1)
    {
        ...
        for(ForCount = 0; ForCount < 8; ++ForCount)
        {
            ...
            first = first >> 1;
            if(ForCount == 0 && (second & 1) == 1)
            {
                CutRoll[0] = 1;
            }
            else if(ForCount == 4 && (second & 1) == 1) 
            {
                CutRoll[1] = 1;
            }
            else if(ForCount == 1)
            {
                if(second & 1)
                {
                    PINCount[1][ForCount] = PINCount[1][ForCount] + CutRoll[0];
                    CutRoll[0] = 0;
                }
            }
            else if(ForCount == 5)
            {
                if(second & 1)
                {
                    PINCount[1][ForCount] = PINCount[1][ForCount] + CutRoll[1];
                    CutRoll[1] = 0;
                }
            }
            else if(ForCount == 2 || ForCount == 3)
            {
                ;
            }
            else
...
*/
        ...
        pfile = fopen("/home/pi/works/tsw100/barcode","r");
        ...
        //MechSTART 
        pfile = fopen(UPLoadFile, "a");
        fprintf(pfile, "%s\t%s\t%s\t%ld\t%ld\t0\t%s\t8\t%s\t%s\t0\t0\t0\t%02d\n", ISNo, ManagerCard, CountNo, PINCount[0][6], (long)now.tv_sec,
                                                                                  inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr), 
                                                                                  MachineCode, UserNo, MechSTART);
        fclose(pfile);
        ...
        while(MasterFlag)
        {
            ...
            write_value = i2c_smbus_write_byte_data(fd, OUT_P0, 0x00);
            printf("1  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, INV_P0, 0xff);
            printf("2  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, CONFIG_P0, 0xff);
            printf("3  %d\n", write_value);

            write_value = i2c_smbus_write_byte_data(fd, OUT_P1, 0x00);
            printf("4  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, INV_P1, 0xf3);
            printf("5  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, CONFIG_P1, 0xf3); 
            printf("6  %d\n", write_value);

            tempInterrupt = i2c_smbus_read_byte_data(fd, IN_P0);
            tempInterrupt = i2c_smbus_read_byte_data(fd, IN_P1);
            close(fd);
            ...
            printf("9  %d\n", write_value);
            i2c_smbus_write_byte_data(fd, INV_P1, 0x7f);
            printf("10 %d\n", write_value);
            i2c_smbus_write_byte_data(fd, CONFIG_P1, 0x7f);
            tempInterrupt = i2c_smbus_read_byte_data(fd, IN_P0);
            tempInterrupt = i2c_smbus_read_byte_data(fd, IN_P1);
            close(fd);
            ...
            i2c_smbus_write_byte_data(fd, OUT_P0, 0x00);
            printf("11 %d\n", write_value);
            i2c_smbus_write_byte_data(fd, INV_P0, 0x0f4);
            printf("12 %d\n", write_value);
            i2c_smbus_write_byte_data(fd, CONFIG_P0, 0xf4);
        ...
        Log(s, __func__, __LINE__, " PIN_22 call me back\n");
            ...
            if(MasterFlag == 0 && isResetButtonPush == 1) 
            {
                pfile = fopen(UPLoadFile, "a"); 
                fprintf(pfile, "%s\t%s\t%s\t%ld\t%ld\t0\t%s\t8\t%s\t%s\t0\t0\t0\t%02d\n", ISNo, ManagerCard, CountNo, PINCount[0][6], (long)now.tv_sec,
                                                                                          inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr), 
            ...
            /*if(MasterFlag)
            {
                ...
                while(1)
                {
                    ...
                    if(strncmp(tempString, "XXX", 3) == 0)
                    {
                        ...
                        fprintf(pfile, "%s\t%s\t%s\t%ld\t%ld\t0\t%s\t8\t%s\t%s\t0\t0\t0\t%02d\n", ISNo, ManagerCard, CountNo, PINCount[0][6], (long)now.tv_sec,
                                                                                                  inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr), 
                                                                                                  MachineCode, UserNo, MechiUNLOCK);
                        fclose(pfile);
