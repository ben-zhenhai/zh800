        ...
        if(WriteFileCount == 299 || PrintLeftLog == 1)
        {
            ...
            for(ForCount = 0; ForCount <= WriteFileCount; ForCount++)
            {
                ...
                if(LogString[ForCount] == '\n')
                {
                    ...
                    ts = *localtime(&now.tv_sec);
                    strftime(buff, sizeof(buff), "%Y/%m/%d_%H:%M:%S", &ts);
                    fprintf(pfile,"m800\t%s\t%ld\t",buff,PINCount[1][6]);
                }
            }      
...
void * zhINTERRUPT1(void * argument)
{
    while(zhInterruptEnable1)
    {
        ...
        for(ForCount = 0; ForCount < 8; ++ForCount)
        {
            ...
            second = second >> 1;
            if(ForCount == 0 && (first & 1) == 1)
            {
                CutRoll[0] = 1;
            }
            else if(ForCount == 1 && (first & 1) == 1) 
            {
                CutRoll[1] = 1;
            }
            else if(ForCount == 2 && (first & 1) == 1)
            {
                PINCount[0][0] = PINCount[0][0] + CutRoll[0];
                PINCount[0][1] = PINCount[0][1] + CutRoll[1];
...
*/
        ...
        pfile = fopen("/home/pi/works/m800/barcode","r");
        ...
        //MechSTART
        pfile = fopen(UPLoadFile, "a");
        fprintf(pfile, "%s\t%s\t%s\t%ld\t%ld\t0\t%s\t16\t%s\t%s\t0\t0\t0\t%02d\n", ISNo, ManagerCard, CountNo, PINCount[1][6], (long)now.tv_sec,
                                                                                  inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr), 
        ...
        while(MasterFlag)
        {
            ...
            printf("1  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, INV_P0, 0x07);
            printf("2  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, CONFIG_P0, 0x07);
            printf("3  %d\n", write_value);

            write_value = i2c_smbus_write_byte_data(fd, OUT_P1, 0x00);
            printf("4  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, INV_P1, 0x7f);
            printf("5  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, CONFIG_P1, 0x7f); 
            printf("6  %d\n", write_value);
