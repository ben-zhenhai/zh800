    ...
    while(FileFlag)   
    {
        ...
        if(updateFlag == 1)
        {
...
#ifdef PrintInfo
            printf("%ld %ld %ld %ld %ld\n", Count[Good], Count[CX_Bad], Count[DX_Bad],  Count[LC_Over], Count[Total]);
#endif
            pthread_mutex_lock(&Mutexlinklist);
            InputNode *p = list;
            ...
            if(p != NULL)
            {
                ...
                //if(WriteFileCount == 0 || ListHaveNode == 0)
                //{
                    pfile = fopen(p->UPLoadFile, "a");
                    for(ForCount = 0; ForCount < (AgeCountNumber-1) ; ForCount++)
                    {
                        if((ExCount[ForCount] != Count[ForCount]) && ForCount == 0)
                        {
                            fprintf(pfile, "%s\t%s\t%s\t%ld\t%ld\t0\t%s\t1\t%s\t%s\t%lf\t%lf\t%lf\n", 
                                                                                   p->ISNo, p->ManagerCard, p->CountNo, Count[Good], (long)now.tv_sec,
                                                                                   inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr), 
                                                                                   p->MachineCode, p->UserNo, cx, dx, lc);
                            break; // only have one in all for loop
                        }else if(ExCount[ForCount] != Count[ForCount])
                        {
                            fprintf(pfile, "%s\t%s\t%s\t%ld\t%ld\t%ld\t%s\t%d\t%s\t%s\t%lf\t%lf\t%lf\n", 
...
int main(int argc ,char *argv[])
{
    ...
    */
    ...
    while(1)
    {
        ...
        sleep(1);
        memset(node->ManagerCard, 0, sizeof(char)*InputLength);
        strcpy(node->ManagerCard, "BL20123456790");
        digitalWrite (WiringPiPIN_15, HIGH);
        ...
        //[vers| when add a new lot will set to 1]
        ListHaveNode = 1;
...
void * FTPFunction(void *argument)
{
    ...
    //while(ListHaveNode){
        ...
        strcpy(UPLoadFile_3, UPLoadFile_2);
        printf("UPLoadFile:%s\n", UPLoadFile_3);
        strcat(Remote_url,UPLoadFile_3);
        if(stat(UPLoadFile_3, &file_info)) {
            printf("Couldnt open %s: %s\n", UPLoadFile_3, strerror(errno));
