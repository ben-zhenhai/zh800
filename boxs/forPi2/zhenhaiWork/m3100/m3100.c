...
int SetI2cConfig()
{
    ...
    i2c_smbus_write_byte_data(fd, OUTP0, 0x00);
    i2c_smbus_write_byte_data(fd, INVP0, 0xfc);
    i2c_smbus_write_byte_data(fd, CONFIGP0, 0xfc);

    i2c_smbus_write_byte_data(fd, OUTP1, 0x00);
    i2c_smbus_write_byte_data(fd, INVP1, 0xf8);
    i2c_smbus_write_byte_data(fd, CONFIGP1, 0xf8);
    i2c_smbus_read_byte_data(fd, INP0);
    i2c_smbus_read_byte_data(fd, INP1);
    close(fd);
                    ...
                    if(forCount == GOODCOUNT)
                    {
                        ...
                        else
                        {
                            fprintf(filePtr, "%s %s %s %ld %ld 0 %s %d %s %s 0 0 0 %02d\n", 
                                                                ...
                                                                (Count[forCount]-Count[forCount+1]) - (ExCount[forCount]-ExCount[forCount+1]),
            ...
            for(forCount = 0; forCount < 8; ++forCount)
            {
                switch(forCount)
                {
                    case 2:
                        ...
                        //Count[24] = Count[24];
                        Count[27] = Count[27] + CutRoll[0];
                        Count[29] = Count[29] + CutRoll[1];
                        CutRoll[0] = 0;
                        CutRoll[1] = 0;
                        break;
                    case 3:
                    ...
                    case 6:
                    case 7:
                        Count[16+forCount] = Count[16+forCount] + (first & 1);
                        break;
                    default:
                        ;
                }
