...
int SetI2cConfig()
{
    ...
    i2c_smbus_write_byte_data(fd, INVP0, 0xd8);
    i2c_smbus_write_byte_data(fd, CONFIGP0, 0xd8);
    ...
    i2c_smbus_write_byte_data(fd, OUTP0, 0x00);
    i2c_smbus_write_byte_data(fd, INVP0, 0x3f);
    i2c_smbus_write_byte_data(fd, CONFIGP0, 0x3f);
        ...
        if(I2CEXValue[2] != x || I2CEXValue[3] != y)
        {
            ...
            for(forCount = 0; forCount < 8; ++forCount)
            {
                ...
                }else
                {
                    if((first & 1))
                    {
                        Count[16+forCount] = Count[16+forCount] + CutRoll[forCount-4];
                        CutRoll[forCount-5] = 0;
                    }
                }
                first = first >> 1;
                Count[24+forCount] = Count[24+forCount] + (second & 1);
                second = second >> 1;
            }
            WatchdogResetFlag = 1;
        }
