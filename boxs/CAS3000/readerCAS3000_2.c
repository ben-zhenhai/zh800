...
pthread_mutex_t mutex, mutex_2, mutex_3, mutex_log, mutexFTP, mutexFile, MutexLinkList;
...
void * WatchDogFunction(void *argument)
{
    pthread_t interruptThread1, fileThread, ftpThread;
    int rc = 0;
...
void * zhLogFunction(void *argument)
{
    ...
    fprintf(pfile,"CAS3000\t%s\t0\t", buff);
    fclose(pfile);
    //printf("%s\n",LogFileLocation);
    while(*shm_pop != '*')
    {
        ...
        if(WriteFileCount == 299 || PrintLeftLog == 1)
        {
            ...
            for(ForCount = 0; ForCount <= WriteFileCount; ForCount++)
            {
                ...
                if(LogString[ForCount] == '\n')
                {
                    ...
                    ts = *localtime(&now.tv_sec);
                    strftime(buff, sizeof(buff), "%Y/%m/%d_%H:%M:%S", &ts);
                    fprintf(pfile,"CAS3000\t%s\t%ld\t",buff,PINCount[1][6]);
                }
            }      
    ...
    Log(s, __func__, __LINE__, " WatchDog wake\n");
        ...
        for(ForCount = 0; ForCount < 2; ++ForCount)
        {
            for(ForCount2 = 0; ForCount2 < 8; ++ForCount2)
            {
                ...
                {
                    fprintf(pfile, "%s\t%s\t%s\t%ld\t%ld\t0\t%s\t%d\t%s\t%s\t0\t0\t0\t%02d\t%02d\n", ISNo, ManagerCard, CountNo, PINCount[1][6], (long)now.tv_sec,
...
int main(int argc, char *argv[])
{
    ...
    i2c_smbus_write_byte_data(fd, INV_P1, 0xFC);
    i2c_smbus_write_byte_data(fd, CONFIG_P1, 0xFC); 
    ...
    while(1)
    {
        ...
        printf("Get button event!! Ready to cancel barcode\n");
        pthread_cancel(basrcodeInputThread);
    ...
    while(1)
    {
        ...
        while(1)
        {
            ...
            if(strncmp(tempString, "YYY", 3) == 0)
            {
                ...
                memcpy(node->ISNo, tempPtr, sizeof(char)*tempString-2);
        ...
        while(1)
        {
            memset(tmepString, 0, sizeof(char)*InputLength);
            gets(tempString);
            if(strncmp(tempString, "QQQ", 3) == 0)
            {
                memset(node->ManagerCard, 0, sizeof(char)*InputLength);
                tempPtr = tempString + 3;
                memcpy(node->ManagerCard, tempPtr, sizeof(char)*tempString-2);
                digitalWrite (WiringPiPIN_15, HIGH);
        ...
        while(1)
        {
            ...
            {
                memset(node->CountNo, 0, sizeof(char)*InputLngth);
                tempPtr =tempString+3;
        ...
        printf("%s %s %s %s %s %s\n", node->ISNo, node->NabagerCard, node->MachineCode, node->UserNo, node->CountNo, node->UPLoadFile);
...
*/
        ...
        while(MasterFlag)
        {
            ...
            printf("4  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, INV_P1, 0xFC);
            printf("5  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, CONFIG_P1, 0xFC); 
            printf("6  %d\n", write_value);
            ...
            if(MasterFlag)
            {
                ...
                while(1)
                {
                    ...
                    if(strncmp(tempString, "XXX", 3) == 0)
                    {
                        ...
                        pfile = fopen(UPLoadFile, "a");
                        fprintf(pfile, "%s\t%s\t%s\t%ld\t%ld\t0\t%s\t8\t%s\t%s\t0\t0\t0\t%02d\t%02d\n", ISNo, ManagerCard, CountNo, PINCount[0][6], 
                                                                                                  (long)now.tv_sec,
                                                                                                  inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr), 
