...
#define RS232SenderLength 26
#define RS232ReceiverLength 62
...
long transFormatLONG(unsigned char x);
...
void * SerialFunction(void *argument)
{
    ...
    while(SerialThreadFlag)
    {
        ...
        for(forCount = 0; forCount < RS232SenderLength; ++forCount)
        {
        ...
        if(getReady == 1 && stringCount == 85)
        {
            ...
            Count[InCome] = transFormatLONG(arrayReceiver[4]) + transFormatLONG(arrayReceiver[5])*256; 
            Count[OpenBad] = transFormatLONG(arrayReceiver[8]) + transFormatLONG(arrayReceiver[9])*256; 
            Count[ShortBad] = transFormatLONG(arrayReceiver[12]) + transFormatLONG(arrayReceiver[13])*256; 
            Count[TitleBad] = transFormatLONG(arrayReceiver[16]) + transFormatLONG(arrayReceiver[17])*256; 
            Count[AGSHBad] = transFormatLONG(arrayReceiver[20])+ transFormatLONG(arrayReceiver[21])*256; 
            Count[OutputBad] = transFormatLONG(arrayReceiver[24]) + transFormatLONG(arrayReceiver[25])*256; 
            Count[CaptainBad] = transFormatLONG(arrayReceiver[28]) + transFormatLONG(arrayReceiver[29])*256; 
            Count[LCUpperBad] = transFormatLONG(arrayReceiver[32]) + transFormatLONG(arrayReceiver[33])*256; 
            Count[LCLowerBad] = transFormatLONG(arrayReceiver[36]) + transFormatLONG(arrayReceiver[37])*256; 
            Count[DxBad] = transFormatLONG(arrayReceiver[40]) + transFormatLONG(arrayReceiver[41])*256; 
            Count[Retest] = transFormatLONG(arrayReceiver[44]) + transFormatLONG(arrayReceiver[45])*256; 
            Count[TopBad] = transFormatLONG(arrayReceiver[48]) + transFormatLONG(arrayReceiver[49])*256; 
            ...
            for(forCount = 0; forCount < AgeCountNumber ; forCount++)
            {
                if((ExCount[forCount] != Count[forCount]) && forCount == 0)
                {
                    ...
                    fprintf(pfile, "%s %s %s %ld %ld 0 %s 1 %s %s 0 0 0 %02d\n", 
                                                                           ...
                                                                           Count[Good] - ExCount[Good], (long)now.tv_sec,
                                                                           inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr), 
                                                                           p->MachineCode, p->UserNo, MachRUNNING);
#else
                    fprintf(pfile, "%s %s %s %ld %ld 0 %s 1 %s %s 0 0 0 %02d\n", 
    ...
    */
    ...
    while(1)
    {
        ...
        */
        while(1)
        {
            ...
            if(flagFailPass == 0 && stringLength > 0)
            {
                ...
                unsigned long goodCount = (unsigned long)atoi(node->CountNo);
                if(goodCount > 0)
                {
        ...
        pfile = fopen("/home/pi/works/acg/barcode","r");
        ...
        */
        ...
        //[vers| avoid no data income cause]
...
void * WatchDogFunction(void *argument)
{
    pthread_t SerialThread, TelnetControlThread, FTPThread;
    int rc = 0;
    FILE *fptr;
    struct timeval now;
    struct ifreq ifr;
