        ...
        if(WriteFileCount == 299 || PrintLeftLog == 1)
        {
            ...
            for(ForCount = 0; ForCount <= WriteFileCount; ForCount++)
            {
                ...
                if(LogString[ForCount] == '\n')
                {
                    ...
                    ts = *localtime(&now.tv_sec);
                    strftime(buff, sizeof(buff), "%Y/%m/%d_%H:%M:%S", &ts);
                    fprintf(pfile,"taicon\t%s\t%ld\t",buff,PINCount[0][7]);
                }
            }      
...
*/
        ...
        pfile = fopen("/home/pi/works/taicon/barcode","r");
        ...
        while(MasterFlag)
        {
            ...
            printf("1  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, INV_P0, 0x80);
            printf("2  %d\n", write_value);
            write_value = i2c_smbus_write_byte_data(fd, CONFIG_P0, 0x80);
            printf("3  %d\n", write_value);
        ...
        if(FTPCount == 0 || FTPFlag == 0 || size > 100000)
        {
            ...
            if(file_info.st_size > 0)
            {
                ...
                else if(proc == 0)
                {
                    ...
                    //strcpy(filePath, "/home/pi/works/taicon/");
