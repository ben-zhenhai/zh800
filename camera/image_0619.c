...
void Integral(int limit)
{
    ...
    }else
    {
        ...
        for(ForCount = 0; ForCount < row; ForCount++)
        {
            ...
            if(valueY == 0 && SumY[ForCount] > ((column*3)/4))
            {
                YUpperBoundPos = ForCount;
                valueY = SumY[ForCount];
                YUpperBoundvalue = valueY;
            }
            else if(valueY != 0 && SumY[ForCount] < (column)/8 && YLowerBoundPos == 0 && YLowerBoundvalue == 0)
            {
        ...
        for(ForCount = 0; ForCount < column; ForCount++)
        {
            CvScalar s = cvGet2D(dst,YUpperBoundPos,ForCount); 
            s.val[0] = 70;
            cvSet2D(dst, YUpperBoundPos, ForCount, s);
        }
    }
}
